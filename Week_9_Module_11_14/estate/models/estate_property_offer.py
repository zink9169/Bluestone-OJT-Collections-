...
class EstatePropertyOffer(models.Model):
    ...
    # Add this method in the EstatePropertyOffer class:
    @api.model
    def create(self, vals):
        # Check 1: Prevent lower price offers
        if 'property_id' in vals and 'price' in vals:
            property = self.env['estate.property'].browse(vals['property_id'])

            # Get existing offers for this property
            existing_offers = property.offer_ids.filtered(lambda o: o.status in ['pending', 'accepted'])

            if existing_offers:
                # Find the highest existing offer price
                max_existing_price = max(existing_offers.mapped('price'))

                # Compare with new offer price
                if vals['price'] < max_existing_price:
                    raise UserError(
                        f"Cannot create an offer with price {vals['price']} "
                        f"which is lower than existing offer price {max_existing_price}"
                    )

        # Create the offer
        offer = super().create(vals)

        # Check 2: Update property state to 'offer_received'
